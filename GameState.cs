...
public class GameState : IGameState
{
    ...
    {
        get { return map[location.Row, location.Col]; }
    }
    ...
    public Location GetDestination(Location location, Direction direction)
    {
        ...
        var row = (location.Row + delta.Row) % Height;
        if (row < 0) row += Height; // because the modulo of a negative number is negative

        var col = (location.Col + delta.Col) % Width;
        if (col < 0) col += Width;
    ...
    {
        var d_row = Math.Abs(loc1.Row - loc2.Row);
        d_row = Math.Min(d_row, Height - d_row);

        var d_col = Math.Abs(loc1.Col - loc2.Col);
    ...
    public bool GetIsVisible(Location loc)
    {
        ...
        for (var r = -1 * squares; r <= squares; ++r)
        {
            for (var c = -1 * squares; c <= squares; ++c)
            {
                var square = r * r + c * c;
